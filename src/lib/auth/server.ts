...
const CUSTOM_SITE_ORIGINS: string[] = [
  "https://dogtrainingwithemily.com",
  "https://www.dogtrainingwithemily.com",
  "http://dogtrainingwithemily.com",
  "http://www.dogtrainingwithemily.com",
];

function isAllowedAuthHost(hostname: string): boolean {
  const h = hostname.toLowerCase();
  return (
    h === "dogtrainingwithemily.com" ||
    h === "www.dogtrainingwithemily.com" ||
    h.endsWith(".grok.me") ||
...
const baseURL = explicitBaseURL ?? {
  ...
  allowedHosts: [
    ...previewAllowedHosts,
    "dogtrainingwithemily.com",
    "www.dogtrainingwithemily.com",
    "*.grok.me",
    "localhost",
    "127.0.0.1",
    "[::1]",
...
// Missing entries here surface as FORBIDDEN "Invalid origin". Trust the
// live request Origin/Referer only when the host is this app.
const trustedOrigins = async (request?: Request): Promise<string[]> => {
  ...
  const baseOrigin = originOf(explicitBaseURL);
  if (baseOrigin) origins.add(baseOrigin);
  else if (explicitBaseURL) origins.add(explicitBaseURL);
  ...
  if (request) {
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        if (isAllowedAuthHost(url.hostname)) origins.add(url.origin);
      } catch {
        /* ignore malformed */
      }
    }
